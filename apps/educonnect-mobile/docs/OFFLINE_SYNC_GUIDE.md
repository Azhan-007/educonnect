...
The EduConnect mobile app automatically queues failed API requests and retries them with exponential backoff when the network becomes available. This enables users to interact with the app even during network outages, with mutations replayed seamlessly when connectivity is restored.

**Supported Operations:**
- ✅ Attendance marking (mark present/absent for students)
- ✅ Assignment submissions (submit coursework)
- ✅ Fee/payment operations (record payments, create payment orders)
- ✅ Admin fee management (record manual payments, create fee templates)
...
```
educonnect.offlineMutationQueue.v1
```
...
```
Attempt 0 → Queue immediately
↓
Attempt 1 → Retry at now + 2^0 * 1000ms = 1 second
↓
Attempt 2 → Retry at now + 2^1 * 1000ms = 2 seconds
↓
Attempt 3 → Retry at now + 2^2 * 1000ms = 4 seconds
↓
Attempt 4 → Retry at now + 2^3 * 1000ms = 8 seconds
↓
Attempt 5 → Retry at now + 2^4 * 1000ms = 16 seconds
↓
Attempt 6+ → Cap at 120 seconds; manual intervention required
```
...
| Endpoint | Method | Queueable |
|----------|--------|-----------|
| `/payments/create-order` | POST | ✅ Yes |
| `/payments` | POST | ✅ Yes |
...
3. Student enters solution → "Submit" button tapped
...
2. **Restart app:** Triggers AppState listener → queue flush
...
- **AppState listener:** Immediate on background → foreground
...
AsyncStorage.getItem('educonnect.offlineMutationQueue.v1')
...
4. Mark 3–5 students as present/absent
...
- [offlineSyncQueue.ts](./offlineSyncQueue.ts) — Core queue implementation
- [attendanceService.ts](./attendanceService.ts) — Attendance + queue integration
- [assignmentService.ts](./assignmentService.ts) — Assignment + queue integration
- [feesService.ts](./feesService.ts) — Fee payment + queue integration
- [adminFeeService.ts](./adminFeeService.ts) — Admin fee + queue integration
- [../app/_layout.tsx](../app/_layout.tsx) — AppState listener + periodic timer
- [../app/login.tsx](../app/login.tsx) — Login-triggered flush
