import type { TranslationKeys } from "../index";

const kn: TranslationKeys = {
  nav: {
    dashboard: "ಡ್ಯಾಶ್‌ಬೋರ್ಡ್",
    students: "ವಿದ್ಯಾರ್ಥಿಗಳು",
    teachers: "ಶಿಕ್ಷಕರು",
    classes: "ತರಗತಿಗಳು",
    attendance: "ಹಾಜರಾತಿ",
    results: "ಫಲಿತಾಂಶಗಳು",
    fees: "ಶುಲ್ಕಗಳು",
    library: "ಗ್ರಂಥಾಲಯ",
    timetable: "ಸಮಯ ಪಟ್ಟಿ",
    events: "ಕಾರ್ಯಕ್ರಮಗಳು",
    reports: "ವಿಶ್ಲೇಷಣೆ",
    settings: "ಸೆಟ್ಟಿಂಗ್‌ಗಳು",
    subscription: "ಚಂದಾದಾರಿಕೆ",
    branding: "ಬ್ರ್ಯಾಂಡಿಂಗ್",
    api: "API",
    schools: "ಶಾಲೆಗಳು",
    logout: "ಲಾಗ್ ಔಟ್",
  },
  sections: {
    overview: "ಅವಲೋಕನ",
    academics: "ಶೈಕ್ಷಣಿಕ",
    finance: "ಹಣಕಾಸು",
    resources: "ಸಂಪನ್ಮೂಲಗಳು",
    settings: "ಸೆಟ್ಟಿಂಗ್‌ಗಳು",
    admin: "ಆಡಳಿತ",
  },
  common: {
    save: "ಉಳಿಸಿ",
    cancel: "ರದ್ದುಮಾಡಿ",
    delete: "ಅಳಿಸಿ",
    edit: "ಸಂಪಾದಿಸಿ",
    add: "ಸೇರಿಸಿ",
    search: "ಹುಡುಕಿ",
    filter: "ಫಿಲ್ಟರ್",
    export: "ರಫ್ತು",
    import: "ಆಮದು",
    loading: "ಲೋಡ್ ಆಗುತ್ತಿದೆ...",
    noData: "ಡೇಟಾ ಲಭ್ಯವಿಲ್ಲ",
    confirm: "ದೃಢೀಕರಿಸಿ",
    back: "ಹಿಂದಕ್ಕೆ",
    next: "ಮುಂದೆ",
    previous: "ಹಿಂದಿನ",
    submit: "ಸಲ್ಲಿಸಿ",
    reset: "ರೀಸೆಟ್",
    close: "ಮುಚ್ಚಿ",
    refresh: "ರಿಫ್ರೆಶ್",
    download: "ಡೌನ್‌ಲೋಡ್",
    upload: "ಅಪ್‌ಲೋಡ್",
    view: "ವೀಕ್ಷಿಸಿ",
    actions: "ಕ್ರಿಯೆಗಳು",
    status: "ಸ್ಥಿತಿ",
    date: "ದಿನಾಂಕ",
    name: "ಹೆಸರು",
    email: "ಇಮೇಲ್",
    phone: "ಫೋನ್",
    address: "ವಿಳಾಸ",
    selectAll: "ಎಲ್ಲಾ ಆಯ್ಕೆ ಮಾಡಿ",
    deselectAll: "ಎಲ್ಲಾ ಆಯ್ಕೆ ರದ್ದು",
    showing: "ತೋರಿಸುತ್ತಿದೆ",
    of: "ರಲ್ಲಿ",
    entries: "ನಮೂದುಗಳು",
    rowsPerPage: "ಪ್ರತಿ ಪುಟಕ್ಕೆ ಸಾಲುಗಳು",
    yes: "ಹೌದು",
    no: "ಇಲ್ಲ",
    active: "ಸಕ್ರಿಯ",
    inactive: "ನಿಷ್ಕ್ರಿಯ",
  },
  auth: {
    login: "ಲಾಗಿನ್",
    forgotPassword: "ಪಾಸ್‌ವರ್ಡ್ ಮರೆತಿರಾ?",
    resetPassword: "ಪಾಸ್‌ವರ್ಡ್ ರೀಸೆಟ್ ಮಾಡಿ",
    emailLabel: "ಇಮೇಲ್ ವಿಳಾಸ",
    passwordLabel: "ಪಾಸ್‌ವರ್ಡ್",
    loginButton: "ಸೈನ್ ಇನ್ ಮಾಡಿ",
    sendResetLink: "ರೀಸೆಟ್ ಲಿಂಕ್ ಕಳುಹಿಸಿ",
    backToLogin: "ಲಾಗಿನ್‌ಗೆ ಹಿಂತಿರುಗಿ",
    welcome: "ಮರಳಿ ಸ್ವಾಗತ",
    signInSubtitle: "ನಿಮ್ಮ SuffaCampus ಖಾತೆಗೆ ಸೈನ್ ಇನ್ ಮಾಡಿ",
  },
  dashboard: {
    title: "ಡ್ಯಾಶ್‌ಬೋರ್ಡ್",
    totalStudents: "ಒಟ್ಟು ವಿದ್ಯಾರ್ಥಿಗಳು",
    totalTeachers: "ಒಟ್ಟು ಶಿಕ್ಷಕರು",
    totalClasses: "ಒಟ್ಟು ತರಗತಿಗಳು",
    attendanceRate: "ಹಾಜರಾತಿ ದರ",
    feeCollection: "ಶುಲ್ಕ ಸಂಗ್ರಹ",
    recentActivity: "ಇತ್ತೀಚಿನ ಚಟುವಟಿಕೆ",
    upcomingEvents: "ಮುಂಬರುವ ಕಾರ್ಯಕ್ರಮಗಳು",
    quickActions: "ತ್ವರಿತ ಕ್ರಿಯೆಗಳು",
  },
  students: {
    title: "ವಿದ್ಯಾರ್ಥಿಗಳು",
    addStudent: "ವಿದ್ಯಾರ್ಥಿ ಸೇರಿಸಿ",
    editStudent: "ವಿದ್ಯಾರ್ಥಿ ಸಂಪಾದಿಸಿ",
    studentDetails: "ವಿದ್ಯಾರ್ಥಿ ವಿವರಗಳು",
    rollNumber: "ರೋಲ್ ನಂಬರ್",
    class: "ತರಗತಿ",
    section: "ವಿಭಾಗ",
    gender: "ಲಿಂಗ",
    dob: "ಹುಟ್ಟಿದ ದಿನಾಂಕ",
    guardian: "ಪೋಷಕರ ಹೆಸರು",
    guardianPhone: "ಪೋಷಕರ ಫೋನ್",
    bloodGroup: "ರಕ್ತದ ಗುಂಪು",
    admissionDate: "ಪ್ರವೇಶ ದಿನಾಂಕ",
  },
  teachers: {
    title: "ಶಿಕ್ಷಕರು",
    addTeacher: "ಶಿಕ್ಷಕರನ್ನು ಸೇರಿಸಿ",
    editTeacher: "ಶಿಕ್ಷಕರನ್ನು ಸಂಪಾದಿಸಿ",
    subject: "ವಿಷಯ",
    qualification: "ಅರ್ಹತೆ",
    experience: "ಅನುಭವ",
    joiningDate: "ಸೇರಿದ ದಿನಾಂಕ",
    salary: "ವೇತನ",
    department: "ವಿಭಾಗ",
  },
  fees: {
    title: "ಶುಲ್ಕಗಳು",
    feeType: "ಶುಲ್ಕ ಪ್ರಕಾರ",
    amount: "ಮೊತ್ತ",
    dueDate: "ಅಂತಿಮ ದಿನಾಂಕ",
    paid: "ಪಾವತಿಸಲಾಗಿದೆ",
    pending: "ಬಾಕಿ",
    overdue: "ಮುಕ್ತಾಯವಾಗಿದೆ",
    totalCollected: "ಒಟ್ಟು ಸಂಗ್ರಹ",
    totalPending: "ಒಟ್ಟು ಬಾಕಿ",
    paymentHistory: "ಪಾವತಿ ಇತಿಹಾಸ",
    recordPayment: "ಪಾವತಿ ದಾಖಲಿಸಿ",
  },
  attendance: {
    title: "ಹಾಜರಾತಿ",
    present: "ಹಾಜರಿದ್ದಾರೆ",
    absent: "ಗೈರುಹಾಜರಿ",
    late: "ತಡವಾಗಿ",
    excused: "ರಜೆ",
    markAttendance: "ಹಾಜರಾತಿ ದಾಖಲಿಸಿ",
    attendanceReport: "ಹಾಜರಾತಿ ವರದಿ",
    todayAttendance: "ಇಂದಿನ ಹಾಜರಾತಿ",
    selectDate: "ದಿನಾಂಕ ಆಯ್ಕೆ ಮಾಡಿ",
  },
  settings: {
    title: "ಸೆಟ್ಟಿಂಗ್‌ಗಳು",
    general: "ಸಾಮಾನ್ಯ",
    schoolInfo: "ಶಾಲೆಯ ಮಾಹಿತಿ",
    academicSession: "ಶೈಕ್ಷಣಿಕ ವರ್ಷ",
    notifications: "ಅಧಿಸೂಚನೆಗಳು",
    language: "ಭಾಷೆ",
    selectLanguage: "ಭಾಷೆ ಆಯ್ಕೆ ಮಾಡಿ",
    appearance: "ನೋಟ",
    timezone: "ಸಮಯ ವಲಯ",
    currency: "ಕರೆನ್ಸಿ",
    saveChanges: "ಬದಲಾವಣೆಗಳನ್ನು ಉಳಿಸಿ",
    resetDefaults: "ಡೀಫಾಲ್ಟ್‌ಗಳಿಗೆ ಮರುಹೊಂದಿಸಿ",
  },
};

export default kn;
