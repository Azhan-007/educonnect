...
export default function ResultsPage() {
  ...
  const handleExportPrint = () => {
    ...
    exportToPrint({ title: 'Exam Results', schoolName: currentSchool?.name || 'EduConnect School', headers, rows, filename: `results-${format(new Date(), 'yyyy-MM-dd')}` });
  ...
  const handleExportCSV = () => {
    const headers = ['Student', 'Roll No', 'Class', 'Exam', 'Subject', 'Marks', 'Total', '%', 'Grade', 'Status'];
    const rows = filteredResults.map(r => [r.studentName, r.rollNumber || '', r.className || r.classId, r.examType, r.subject, String(r.marksObtained), String(r.totalMarks), String(r.percentage || 0), r.grade || '', r.status]);
    exportToCSV({ title: 'Exam Results', schoolName: currentSchool?.name || 'EduConnect School', headers, rows, filename: `results-${format(new Date(), 'yyyy-MM-dd')}` });
