...
export default function LibraryPage() {
  ...
  const handleExportPrint = () => {
    ...
    exportToPrint({ title: 'Library Records', schoolName: currentSchool?.name || 'EduConnect School', headers, rows, filename: `library-${format(new Date(), 'yyyy-MM-dd')}` });
  };

  const handleExportCSV = () => {
    const headers = ['Title', 'Author', 'Category', 'ISBN', 'Total Copies', 'Available', 'Issued', 'Status'];
    const rows = filteredBooks.map(b => [b.title, b.author, b.category, b.isbn || '', String(b.totalCopies), String(b.availableCopies), String(b.issuedCount || 0), b.status]);
    exportToCSV({ title: 'Library Records', schoolName: currentSchool?.name || 'EduConnect School', headers, rows, filename: `library-${format(new Date(), 'yyyy-MM-dd')}` });
