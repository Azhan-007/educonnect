...
export default function EventsPage() {
  ...
  const handleExportPrint = () => {
    ...
    exportToPrint({ title: 'School Events', schoolName: currentSchool?.name || 'EduConnect School', headers, rows, filename: `events-${format(new Date(), 'yyyy-MM-dd')}` });
  };
  const handleExportCSV = () => {
    const { headers, rows } = getExportData();
    exportToCSV({ title: 'School Events', schoolName: currentSchool?.name || 'EduConnect School', headers, rows, filename: `events-${format(new Date(), 'yyyy-MM-dd')}` });
  };
