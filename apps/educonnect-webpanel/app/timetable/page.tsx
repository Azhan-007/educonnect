...
export default function TimetablePage() {
  ...
  const handleExportPrint = () => {
    ...
    exportToPrint({ title: 'Timetable', schoolName: currentSchool?.name || 'EduConnect School', headers, rows, filename: `timetable-${format(new Date(), 'yyyy-MM-dd')}` });
  ...
  const handleExportCSV = () => {
    const headers = ['Class', 'Day', 'Periods', 'Subjects'];
    const rows = filteredTimetables.map(t => [t.className || t.classId, t.day, String(t.periods?.length || 0), (t.periods?.map(p => p.subject).join('; ')) || '']);
    exportToCSV({ title: 'Timetable', schoolName: currentSchool?.name || 'EduConnect School', headers, rows, filename: `timetable-${format(new Date(), 'yyyy-MM-dd')}` });
