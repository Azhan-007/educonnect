...
This document explains the Firestore security rules for the EduConnect multi-tenant SaaS platform. The rules enforce:
...
┌─────────────────────────────────────────────────────────────────┐
│                        SUPERADMIN                                │
│  Platform-wide access to ALL schools, subscriptions, billing     │
└─────────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────────┐
...
┌─────────────────────────────────────────────────────────────────┐
│                        PRINCIPAL                                 │
│  Full read, limited write within their school                    │
└─────────────────────────────────────────────────────────────────┘
                              │
              ┌───────────────┴───────────────┐
              ▼                               ▼
┌─────────────────────────┐     ┌─────────────────────────┐
│         STAFF           │     │       ACCOUNTANT        │
│  - Read all data        │     │  - Finance data only    │
│  - Write: attendance,   │     │  - Read/write fees      │
│    assignments, results │     │  - No student/teacher   │
└─────────────────────────┘     └─────────────────────────┘
```
...
| **attendance** | CRUD | CRUD (school) | CRUD (school) | CRU (school) | ❌ |
| **fees** | CRUD | CRUD (school) | CRUD (school) | ❌ | RU (school) |
| **events** | CRUD | CRUD (school) | CRUD (school) | R (school) | R (school) |
| **subscriptions** | CRUD | R (own) | ❌ | ❌ | ❌ |
| **invoices** | CRUD | R (own) | ❌ | ❌ | R (own) |
| **usageRecords** | CRUD | R (own) | ❌ | ❌ | ❌ |
| **classes** | CRUD | CRUD (school) | CRUD (school) | R (school) | R (school) |
| **timetable** | CRUD | CRUD (school) | CRUD (school) | R (school) | R (school) |
| **assignments** | CRUD | CRUD (school) | CRUD (school) | CRUD (own) | ❌ |
| **results** | CRUD | CRUD (school) | CRUD (school) | CRU (school) | ❌ |
...
```javascript
// ✅ SHOULD PASS: Admin reads own school's students
...
// ❌ SHOULD FAIL: Admin tries to read another school's students
...
// ✅ SHOULD PASS: SuperAdmin reads any school's students
...
```javascript
// ✅ SHOULD PASS: Staff creates attendance
...
// ❌ SHOULD FAIL: Staff creates fee record
...
```javascript
// ❌ SHOULD FAIL: Admin creates student in another school
...
// ❌ SHOULD FAIL: Admin changes student's schoolId
...
```javascript
// ❌ SHOULD FAIL: User changes own role
...
// ❌ SHOULD FAIL: Admin creates SuperAdmin
...
```javascript
// ❌ SHOULD FAIL: Deactivated user reads data
...
```javascript
// ❌ SHOULD FAIL: Admin changes subscription plan
...
// ✅ SHOULD PASS: Admin updates school name
...
```javascript
// ✅ SHOULD PASS: Accountant reads fees
...
// ✅ SHOULD PASS: Accountant updates payment status
...
// ❌ SHOULD FAIL: Accountant reads student personal info
...
```javascript
// ✅ SHOULD PASS: Admin acknowledges alert
...
// ❌ SHOULD FAIL: Admin tries to modify alert message
