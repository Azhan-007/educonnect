# EduConnect Design Guidelines

> Internal reference for maintaining visual consistency across the EduConnect admin panel.

---

## Design Principles

1. **Calm over clever** — No gimmicks. Every visual choice should reduce noise, not add it.
2. **Consistent density** — Use the 8pt grid. Spacing should feel rhythmic and predictable.
3. **Hierarchy through weight, not color** — Use font-weight and size to guide attention. Reserve color for status and CTAs.
4. **Flat and functional** — No gradients on backgrounds, no heavy shadows. Depth comes from borders and subtle elevation.
...
- `text-[13px]`, `text-[11px]`, `text-[10px]` — Use `text-sm` or `text-xs` instead
- `font-bold` on UI elements — Use `font-semibold` for headings, `font-medium` for buttons and labels
- Gradient text (`bg-clip-text text-transparent`) — Use solid `text-slate-900`
...
- `rounded-2xl` — Too soft for enterprise UI
- `rounded-3xl` — Never use
...
- `shadow-lg`, `shadow-xl` — Too heavy for a professional panel
- `shadow-blue-500/25` or any colored shadows — Not enterprise standard
- `shadow-inner` — Not used
...
- **No colored icon chips** — Icons inherit the text color
...
- `hover:scale-*`, `active:scale-*`, `hover:-translate-y-*` — No scale transforms on cards/buttons
- `animate-fade-in` on page containers — Pages should render instantly
- `transition-all duration-300` for layout-level effects — Too dramatic
