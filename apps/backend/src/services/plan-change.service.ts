...
export async function executePlanChange(schoolId: string, newPlanName: string, billingCycle: "monthly" | "yearly" = "monthly", performedBy: string): Promise<PlanChangeResult> {
  ...
  if (newPlanName.toLowerCase() === "free") {
    ...
    await createNotification({ schoolId, title: "Plan Changed", message: "Your plan has been changed to Free.", type: "subscription", actionUrl: "/settings/subscription" });
  ...
  if (preview.isDowngrade) {
    ...
    await createNotification({ schoolId, title: "Downgrade Scheduled", message: `Downgrade to ${newPlan.displayName} on ${preview.effectiveDate}.`, type: "subscription", actionUrl: "/settings/subscription" });
