...
<body>
  <div class="container">
    <div class="header"><h1>EduConnect</h1></div>
    <div class="body">${body}</div>
    <div class="footer">
      &copy; ${new Date().getFullYear()} EduConnect. All rights reserved.<br/>
...
export function trialExpiringEmail(schoolName: string, daysLeft: number) {
  const subject = `⏰ Your EduConnect trial expires in ${daysLeft} day(s)`;
  const html = layout(subject, `
    ...
    <a href="https://app.educonnect.in/pricing" class="cta">Upgrade Now</a>
...
export function subscriptionExpiredEmail(schoolName: string) {
  const subject = `🔒 Your EduConnect subscription has expired`;
  const html = layout(subject, `
    ...
    <a href="https://app.educonnect.in/pricing" class="cta">Renew Subscription</a>
  `);
  return { subject, html, text: stripHtml(html) };
}
...
export function paymentReceivedEmail(
  ...
  const subject = `✅ Payment received — ${plan} plan active`;
  const html = layout(subject, `
    ...
    <a href="https://app.educonnect.in/settings/subscription" class="cta">View Subscription</a>
...
export function paymentFailedEmail(schoolName: string, retryCount: number) {
  ...
  const html = layout(subject, `
    ...
    <a href="https://app.educonnect.in/settings/subscription" class="cta">Update Payment Method</a>
...
export function planChangedEmail(
  ...
  const subject = `📋 Plan ${direction.toLowerCase()} — ${oldPlan} → ${newPlan}`;
  const html = layout(subject, `
    ...
    <a href="https://app.educonnect.in/settings/subscription" class="cta">View Details</a>
  `);
  return { subject, html, text: stripHtml(html) };
}
...
export function usageLimitWarningEmail(
  ...
  const subject = `⚠️ ${percentage}% of ${resource} limit used`;
  const html = layout(subject, `
    ...
    <a href="https://app.educonnect.in/pricing" class="cta">Upgrade Plan</a>
  `);
  return { subject, html, text: stripHtml(html) };
}
...
export function feeReminderEmail(
  ...
  const amount = `₹${(amountPaise / 100).toFixed(2)}`;
  const subject = `📄 Fee reminder for ${studentName} — ${amount} due ${dueDate}`;
  const html = layout(subject, `
...
export function welcomeEmail(schoolName: string, adminName: string) {
  const subject = `🎉 Welcome to EduConnect!`;
  const html = layout(subject, `
    <h2>Welcome Aboard!</h2>
    <p>Hi ${adminName},</p>
    <p>Your school <strong>${schoolName}</strong> has been set up on EduConnect. You're all set to start managing your institution.</p>
    <div class="highlight">
    ...
    <a href="https://app.educonnect.in/dashboard" class="cta">Go to Dashboard</a>
