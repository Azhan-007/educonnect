...
import { env } from "./lib/env";          // ← validate env vars first
...
export function buildServer() {
  ...
  // CORS
    openapi: {
      info: {
        ...
        version: "1.0.0",
        contact: { name: "EduConnect Team", url: "https://educonnect.in" },
      },
      servers: [
        { url: "http://localhost:5000", description: "Development" },
        { url: "https://api.educonnect.in", description: "Production" },
      ],
      components: {
        securitySchemes: {
          BearerAuth: {
  ...
  server.register(rateLimit, {
    ...
    keyGenerator: (request) => {
      // Per-tenant rate limiting when authenticated, per-IP otherwise
      return request.schoolId ?? request.ip;
    },
...
async function main() {
  ...
  try {
    ...
    startWorkers();
    server.log.info("⏱️  Background workers started");
