...
**Key Endpoints:**
- `GET /health` — Shallow liveness probe (always 200 if process alive)
- `GET /health/live` — Kubernetes liveness alias
- `GET /health/ready` — Deep readiness probe (checks all dependencies)
- `GET /health/cache` — Cache statistics for monitoring
...
### Status Codes
- **200 OK** — Status is `healthy` or `degraded` (pod can receive traffic)
- **503 Service Unavailable** — Status is `unhealthy` (critical dependency failed; pod should not receive traffic)

### Status Interpretation
| Status | Meaning | HTTP Code | Action |
|--------|---------|-----------|--------|
| `healthy` | All configured dependencies are operational | 200 | ✅ Pod ready |
| `degraded` | One or more optional dependencies unavailable, but service functional | 200 | ⚠️ Pod ready, monitor logs |
| `unhealthy` | Required dependency failed; service cannot operate | 503 | ❌ Pod not ready; Kubernetes will evict |
...
- HTTP 200 — pod ready for traffic
...
**Use Cases:** Multi-instance deployments, medium schools (5k–50k users), high availability required
...
- Status transitions to `degraded` → `unhealthy` after connection timeout (~30s)
...
- HTTP 200 — pod always ready

**Use Cases:** Large deployments (50k–500k+ users), enterprise SaaS, high-performance search required
...
**Behavior:**
- HTTP 503 (unhealthy — required for realtime scaling)
...
while true; do
  for pod in $(kubectl get pods -l app=educonnect-backend -o name); do
...
|----------|-------------|---------|
| `FIRESTORE_PROJECT_ID` | Google Cloud Firestore project ID | `educonnect-prod` |
...
- alert: EduConnectBackendUnhealthy
  expr: health_ready_status{status="unhealthy"} > 0
  for: 1m
  annotations:
    summary: "EduConnect backend unhealthy"

# Alert if all pods degraded for > 5 minutes
- alert: EduConnectBackendDegraded
  expr: avg(health_ready_status{status="degraded"}) == 1
  for: 5m
  annotations:
    summary: "EduConnect backend operating in degraded mode"
```
...
|---|---|---|---|---|
| **Bare** | Single-instance, dev, small schools | ❌ | ❌ | `degraded` (optional deps unavailable) |
| **Redis** | Multi-instance, medium schools, HA | ✅ | ❌ | `healthy` (realtime scaled, search fallback) |
| **Full Stack** | Large scale, enterprise, high-performance | ✅ | ✅ | `healthy` (all optimizations active) |
