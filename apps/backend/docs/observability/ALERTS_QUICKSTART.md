...
- Status -> Rules
- Confirm group `educonnect-critical-endpoints` is present

## 3) Validate metrics are scraped

Run queries in Prometheus expression browser:

- `sum(rate(educonnect_critical_request_duration_seconds_count[5m]))`
- `sum(rate(educonnect_auth_lookup_cache_events_total[5m]))`
- `sum(rate(educonnect_dashboard_query_duration_seconds_count[5m]))`
- `sum(rate(educonnect_overload_shed_requests_total[5m]))`
...
- `EduConnectCriticalEndpointP99High`
- `EduConnectOverloadSheddingDetected`
