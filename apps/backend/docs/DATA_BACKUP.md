...
EduConnect uses **Google Cloud Firestore** as its primary data store. This document
...
gsutil mb -l asia-south1 gs://educonnect-backups
...
EOF
gsutil lifecycle set /tmp/lifecycle.json gs://educonnect-backups
```
...
gcloud scheduler jobs create http firestore-daily-backup \
  ...
  --message-body='{"outputUriPrefix":"gs://educonnect-backups/daily"}'
```
...
gsutil ls gs://educonnect-backups/daily/

# Restore a specific backup (CAUTION: overwrites existing data)
gcloud firestore import gs://educonnect-backups/daily/2025-02-27T02:00:00_12345/
```
...
gcloud firestore import \
  gs://educonnect-backups/daily/2025-02-27T02:00:00_12345/ \
...
|--------------------|-----------|-----------------------------|
| Daily export       | 90 days   | `gs://educonnect-backups/daily/` |
| Weekly snapshot    | 1 year    | `gs://educonnect-backups/weekly/` |
| Pre-migration      | Permanent | `gs://educonnect-backups/migrations/` |
...
gcloud scheduler jobs create http firestore-weekly-backup \
  ...
  --oauth-service-account-email=YOUR_SA@YOUR_PROJECT_ID.iam.gserviceaccount.com \
  --headers="Content-Type=application/json" \
  --message-body='{"outputUriPrefix":"gs://educonnect-backups/weekly"}'
```
...
gsutil lifecycle set /tmp/weekly-lifecycle.json gs://educonnect-backups
```
...
gcloud firestore export gs://educonnect-backups/migrations/pre-v2.0-$(date +%Y%m%d)/
```
...
**Firebase Cloud Storage** (`gs://educonnect-uploads/`).

Cloud Storage has built-in:
- **Object versioning** — enabled on the bucket
- **Cross-region replication** — if using multi-region bucket

```bash
# Enable versioning
gsutil versioning set on gs://educonnect-uploads
```
...
in `gs://educonnect-backups/daily/` within the last 26 hours.
...
conditions:
  - conditionAbsent:
      filter: 'resource.type="gcs_bucket" AND resource.labels.bucket_name="educonnect-backups"'
...
gsutil du -s gs://educonnect-backups/daily/$(date +%Y-%m-%d)*/
```
...
2. Import the pre-migration backup from `gs://educonnect-backups/migrations/`
...
gsutil iam ch group:infra@educonnect.in:objectViewer gs://educonnect-backups
```
