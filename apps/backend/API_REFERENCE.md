# EduConnect Backend — API Reference
...
16. [Admin — Schools (SuperAdmin)](#admin--schools-superadmin)
17. [Exports](#exports)
...
Resolve username → email + role (for mobile login flow).
...
- `search` — search name/admission number
...
- `payment.captured` — activates subscription, creates invoice
- `payment.failed` — increments failure count, logs error
- `refund.created` — marks invoice as refunded, logs audit
...
Deep readiness check — validates Firestore connectivity, system metrics.
